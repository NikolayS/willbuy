...
# The override adds a bind-mount to ZFS alongside the existing named volume.
# NOTE (L-3): Docker Compose appends volume entries from overrides — it does
# not replace the original list.  The bind-mount is appended last, so on
# Linux it shadows the named "db-data" volume (last mount wins).  Postgres
# reads/writes the ZFS-backed bind-mount as intended.  The named volume is
# still created and silently unused; remove it after verifying Postgres health:
#   docker volume rm dev_db-data
run tee "${COMPOSE_OVERRIDE}" > /dev/null <<'OVERRIDE'
# docker-compose.zfs.yml — generated by migrate-pgdata.sh
# Adds a bind-mount to the ZFS dataset at /var/lib/postgresql/data.
# Docker Compose APPENDS this to the db service volume list (does not replace).
# The bind-mount is last, so it shadows the original named "db-data" volume.
# See comment in migrate-pgdata.sh for details (L-3 review finding).
#
